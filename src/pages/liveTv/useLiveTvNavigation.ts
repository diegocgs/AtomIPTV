...
export function useLiveTvNavigation({
  ...
  useEffect(() => {
    ...
    focusedChannelRef.current = Math.min(start, channelCount - 1)
    const id = `lch-${focusedChannelRef.current}`
    setFocusedId(id)
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        ...
        if (chCount > 0) {
          focusedChannelRef.current = Math.min(chCount - 1, Math.max(0, focusedChannelRef.current))
          const id = `lch-${focusedChannelRef.current}`
          setFocusedId(id)
          setLastChannelFocusId(id)
        }
      ...
      if (keyRight) {
        ...
        if (panel === 'categories') {
          ...
          if (chCount > 0) {
            focusedChannelRef.current = Math.min(focusedChannelRef.current, chCount - 1)
            const id = `lch-${focusedChannelRef.current}`
            setFocusedId(id)
            setLastChannelFocusId(id)
          }
      ...
      if (keyDown) {
        ...
        if (panel === 'channels' && chCount > 0) {
          ...
          e.stopPropagation()
          setChannelsNavFocus('list')
          focusedChannelRef.current = Math.min(chCount - 1, focusedChannelRef.current + 1)
          const id = `lch-${focusedChannelRef.current}`
          setFocusedId(id)
          setLastChannelFocusId(id)
          return
        ...
        if (panel === 'categories' && catCount > 0) {
          e.preventDefault()
          e.stopPropagation()
          focusedCategoryRef.current = Math.min(catCount - 1, focusedCategoryRef.current + 1)
          const id = `lcat-${focusedCategoryRef.current}`
          setFocusedId(id)
          setLastCategoryFocusId(id)
      ...
      if (keyUp) {
        ...
        if (panel === 'channels') {
          ...
          if (chCount > 0) {
            setChannelsNavFocus('list')
            focusedChannelRef.current = Math.max(0, focusedChannelRef.current - 1)
            const id = `lch-${focusedChannelRef.current}`
            setFocusedId(id)
            setLastChannelFocusId(id)
          } else {
        ...
        if (panel === 'categories' && catCount > 0) {
          ...
          if (focusedCategoryRef.current <= 0) {
            if (!e.repeat) focusHeaderFirstControl()
            return
          }
          focusedCategoryRef.current = Math.max(0, focusedCategoryRef.current - 1)
          const id = `lcat-${focusedCategoryRef.current}`
          setFocusedId(id)
          setLastCategoryFocusId(id)
          setActiveCatIndex(focusedCategoryRef.current)
